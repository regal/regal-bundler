...
const fillInOpts = (configLocation, userOpts) => {
    ...
    if (c.input.file === undefined) {
        ...
        c.input.file = join(configLocation, "src", inputFile);
    }
    else {
        c.input.file = join(configLocation, c.input.file);
    }
    if (c.output === undefined) {
        c.output = {};
    }
    if (c.output.file === undefined) {
        c.output.file = join(configLocation, makeFileName(userOpts.game.name));
    }
    else {
        c.output.file = join(configLocation, c.output.file);
    }
...
const getPlugins = (config) => {
    ...
    plugins.push(append(bundleFooter, {
